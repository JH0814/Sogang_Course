...
// adding by me --> bitmap_expand
struct bitmap *bitmap_expand(struct bitmap *bitmap, int size){
  size_t new_size = bitmap->bit_cnt + size; // update new size of bitmap
  struct bitmap *new_bitmap = bitmap_create(new_size);
  if(new_bitmap == NULL){ // if creation failed, return NULL
    return NULL;
  }
  for(size_t i = 0; i<bitmap->bit_cnt; i++){ // add existing true to new bitmap
    if(bitmap_test(bitmap, i)){
      bitmap_mark(new_bitmap, i);
    }
  }
  return new_bitmap; // return bitmap
}
