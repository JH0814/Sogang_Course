...
void InitTetris(){
	...
	Rec_root->level = 0;
	Rec_root->recBlockX = 0;
	Rec_root->recBlockY = 0;
	Rec_root->recBlockRotate = 0;
	for(int i = 0; i<HEIGHT; i++){
		for(int j = 0; j<WIDTH; j++){
			Rec_root->recField[i][j] = field[i][j];
		}
	}
	recommend(Rec_root);
...
void DrawBlockWithFeatures(int y, int x, int blockID, int blockRotate){ // shadow와 block 동시에 그리기 위한 함수
	...
	DrawRecommend(Rec_y, Rec_x, Rec_ID, Rec_rot);
}
...
int recommend(RecNode* root) {
	...
	int score;
	RecNode new_node;
	RecNode* child = &new_node;
	int x, y, rot;
	for(rot = 0; rot<4; rot++){
		for(x = -2; x<13; x++){
			if(!CheckToMove(root->recField, nextBlock[root->level], rot, 0, x)) continue;
			for(int i = 0; i<HEIGHT; i++){
				for(int j = 0; j<WIDTH; j++){
			...
			y = 0;
			while(CheckToMove(child->recField, nextBlock[root->level], rot, y, x)){
				y++;
			}
			y--;
			score = AddBlockToField(child->recField, nextBlock[root->level], rot, y, x);
			score += DeleteLine(child->recField);
			if(child->level<VISIBLE_BLOCKS) score += recommend(child);
			if(max < score){
				if(root->level == 0){
					Rec_x = x;
					Rec_y = y;
					Rec_rot = rot;
					Rec_ID = nextBlock[root->level];
					max = score;
				}
			}
		}
