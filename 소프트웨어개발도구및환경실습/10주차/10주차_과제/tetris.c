...
int B,count;
int flag = 0; // recommend Play 확인하는 플래그
...
void BlockDown(int sig){
	...
	if(flag == 1){
		...
		blockY = answer->recBlockY;
		blockX = answer->recBlockX;
		blockRotate = answer->recBlockRotate;
	...
	else if(CheckToMove(field, nextBlock[0], blockRotate, blockY + 1, blockX)){ // CheckToMove에서 1 return 시
		blockY++; // 블록의 Y좌표 증가
...
int modified_recommend(RecNode* root) {
	answer = Rec_root; // answer를 초기화
	modified_make_Tree(Rec_root); // Tree에서 모든 경우의 수 계산하면서 answer 업데이트
	if(answer == Rec_root){
		gameOver = 1;
	}
	else if(answer->parent != NULL){
	...
	for (int rot = 0; rot < 4; rot++) { // 회전 모두 판별
		...
		for (int x = -1; x < WIDTH; x++) { // x 좌표 모두 구분
			if (CheckToMove(root->recField, arr[root->level], rot, y, x) == 1) { // 일단 제일 위에 둘 수 있을 때
				...
				root->child[idx]->recBlockY--;
				// 회전수와 score도 반영, Y좌표의 제곱을 더해줘서 좀 더 아래의 블록으로 이동하도록 구현
				root->child[idx]->recBlockRotate = rot;
				root->child[idx]->score = root->score + getscore(root->child[idx]);
				root->child[idx]->score += root->child[idx]->recBlockY * root->child[idx]->recBlockY;
...
void modified_make_Tree(RecNode* root) { //트리 만들기
	int idx = 0, y = 0;
	if (root->level == 0) { // 루트 노드일 때
		for (int i = 0; i < VISIBLE_BLOCKS; i++) {
			arr[i] = nextBlock[i]; // nextBlock 배열 임시 저장
	...
	int arr_rot[7] = {2, 4, 4, 4, 1, 2, 2}; // 회전 가능 수 모두 저장
	for (int rot = 0; rot < arr_rot[arr[root->level]]; rot++) { // 회전 모두 판별
		y = 0;
		for (int x = -1; x < WIDTH; x++) { // x 좌표 모두 구분
			if (CheckToMove(root->recField, arr[root->level], rot, y, x) == 1) { // 일단 제일 위에 둘 수 있을 때
				for (int i = 0; i < HEIGHT;i++) { // 부모 노드로부터 필드 받아옴
				...
				root->child[idx]->score = root->score + getscore(root->child[idx]);
				root->child[idx]->score += root->child[idx]->recBlockY * root->child[idx]->recBlockY;
				//최대 값 비교 후 최대이면 answer 변경, y좌표가 더 큰 경우도 포함(단 노드의 level이 VISIBLE_BLOCKS-1 이상인 경우만)
				if((answer->score < root->child[idx]->score || root->child[idx]->recBlockY > answer->recBlockY) && root->child[idx]->level >= VISIBLE_BLOCKS - 1){
					answer = root->child[idx];
				}
				else if(root->child[idx]->level >= VISIBLE_BLOCKS - 1){
					idx--;
				}
				idx++;
			}
